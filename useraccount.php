...
$review_str = "select review_content, review_id, review_rating, Destination_name, Destination_id from reviews join Destinations on reviews.review_Destination_id=Destinations.Destination_id where reviews.review_user_id=" . $user_id;
	...
	<div class="container wrapper">
		...
		<div class="row">
			<div class="col-xs-8 col-xs-offset-2">
				...
				<p class="lead">Welcome to your user dashboard! Here you can edit your information, see favorite Destinations, and your own reviews!</p>
			</div>
		</div>
		<div class="row">
			<div class="col-xs-8 col-xs-offset-2">
			...
			<div class="col-xs-6">
				...
				if (isset($items)) {
					...
					foreach ($items_count_value as $id => $qty) {
						//select statement
						$query_str = "SELECT * FROM Destinations WHERE Destination_id=" . $id;
						//execute the query
						$result = $conn->query($query_str);
						$result_row = $result->fetch_assoc();

						$id = $result_row['Destination_id'];
						$img = $result_row['Destination_img'];
						$title = "<a href='Destinationdetails.php?id= " . $result_row['Destination_id'] . "'>" . $result_row['Destination_name'] . "</a>";
					...
					echo "<p class='lead text-center'>You don't have any favorite Destinations!</p>";
				}
				?>
			</div>
			<div class="col-xs-6">
				...
				<?php
					while ($review_row = $review_result->fetch_assoc()) { ?>
						...
						<div class="panel panel-default">
							<div class="panel-heading Destination-list">
								<h3 class="panel-title">
									<a href="Destinationdetails.php?id=<?= $review_row['Destination_id'] ?>"><?= $review_row['Destination_name'] ?></a>
								</h3>
							</div>
